...
export function TestConsole() {
  ...
  const handleStartWorkflow = async () => {
    ...
    try {
      const response = await fetch(`${apiBaseUrl}/api/v1/cbesuperapp/utility/proxy/initial`, {
        method: 'POST',
        headers: {
      ...
      if (data.success) {
        ...
        if (data.to_be_overridden?.overridden_request_body) {
          ...
          data.to_be_overridden.overridden_request_body.forEach((field: { field: string }) => {
  ...
  const handleProcessStep = async () => {
    ...
    setTestSteps(prev =>
      prev.map((s, i) => (i === currentStepIndex ? { ...s, status: 'running' } : s))
    );

    try {
      const response = await fetch(`${apiBaseUrl}/api/v1/cbesuperapp/utility/proxy/process`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
          ...
          <>
            ...
            <div className="p-4 border-b bg-white">
              <h3 className="text-sm font-medium text-gray-700 mb-3">
                {currentStepIndex === 0 ? 'Initial Request' : `Step ${currentStepIndex + 1} Input`}
              </h3>

              <div className="space-y-3 max-h-48 overflow-y-auto">
                {Object.keys(inputValues).length === 0 ? (
                  <p className="text-sm text-gray-500">No input fields required for this step</p>
                ) : (
              ...
              <button
                onClick={currentStepIndex === 0 ? handleStartWorkflow : handleProcessStep}
                disabled={isRunning}
            ...
            <div className="flex-1 p-4 overflow-auto bg-gray-50">
              <h3 className="text-sm font-medium text-gray-700 mb-3">Response History</h3>
        ...
        {testSteps.length === 0 && (
          <div className="flex-1 flex items-center justify-center text-gray-500">
            <div className="text-center">
              ...
              <p className="text-sm">Click &quot;Initialize Test&quot; to begin</p>
            </div>
          </div>
        )}
        ...
        {currentStepIndex === -1 && (
          <div className="flex-1 flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="text-xl font-medium text-gray-800">Workflow Complete!</p>
